...
fn arg_to_string(v: &Value) -> Result<String> {
    match v.type_of() {
        Type::String => Ok(v.as_string().unwrap().to_string()?),
...
fn redis_value_to_js<'js>(ctx: &Ctx<'js>, val: &RedisValue) -> Result<Value<'js>> {
    match val {
        RedisValue::SimpleString(s) | RedisValue::BulkString(s) => s.as_str().into_js(ctx),
        RedisValue::Integer(i) => i.into_js(ctx),
        RedisValue::Float(f) => f.into_js(ctx),
        RedisValue::Bool(b) => b.into_js(ctx),
        RedisValue::Array(arr) => {
            let js_arr = rquickjs::Array::new(ctx.clone())?;
            for (i, item) in arr.iter().enumerate() {
                js_arr.set(i, redis_value_to_js(ctx, item)?)?;
            }
